...
pub fn run(terminal: &mut DefaultTerminal, mut game_state: GameState) -> Result<()> {
    let mut terminal_too_small = false;

    // Min size for single-player: board + 2 for borders
    let min_width = game_state.width + 2;
    let min_height = game_state.height + 2;
    ...
    loop {
        ...
        if crossterm::event::poll(std::time::Duration::from_millis(game_state.speed))? {
            match event::read()? {
                ...
                Event::Resize(w, h) => {
                    terminal_too_small = w < min_width || h < min_height;
                }
                _ => {}
            }
        }
